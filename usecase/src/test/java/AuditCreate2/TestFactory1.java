...
public class TestFactory1 {
    ...
    public Object[] createTests() throws Exception {
        String path = System.getProperty("user.dir") + "/testdata/updated.xlsx";
        ExcelUtility.setExcelPath(path);
        String sheet = "Sheet1";
        int rowCount = ExcelUtility.getRowCount(sheet)+1;
        System.out.println("1111111111"+rowCount);
        Object[] tests = new Object[rowCount - 1]; // Assuming row 0 is header

        for (int i = 1; i <rowCount; i++) {
        	System.out.println(rowCount+"hiiii");
            ...
            tests[i - 1] = new FullAuditFlowSplit1(email, password, auditName, startMM,
                    startDD, startYYYY, endMM, endDD, endYYYY, auditor, stakeholder, organisation);
        }
